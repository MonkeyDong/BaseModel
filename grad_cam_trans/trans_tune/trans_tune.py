...
train_idx, val_idx = next(ss.split(codes[:-33], labels[:-33]))#返回的是序列号
...
#test_xs, test_ys = codes[-11:], labels_vecs[-11:] 
...
with tf.Session() as sess:
    ...
    output = sess.run(predicted, feed_dict=feed)
    labelss = sess.run(labels_, feed_dict=feed)

    print("Test accuracy: {:.4f}".format(test_acc))
    
    #np.save('output.npy',output)
    #np.save('labelss.npy',labelss)
    '''
